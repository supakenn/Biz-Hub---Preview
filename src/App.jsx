...
// =============================================================================
// MICRO-COMPONENT: FallbackScreen
// =============================================================================
...
// =============================================================================
// PANEL COMPONENT: OrderFeed (Left/Top Panel)
// =============================================================================
const OrderFeed = ({
...
// =============================================================================
// PANEL COMPONENT: MenuGrid (Right/Bottom Panel)
// =============================================================================
const MenuGrid = ({
    ...
    <div className="w-full sm:w-[60%] md:w-[65%] shrink-0 sm:shrink sm:h-[100dvh] flex flex-col bg-gray-100 dark:bg-gray-950 max-h-[60dvh] sm:max-h-full overflow-hidden shadow-[0_-4px_10px_-4px_rgba(0,0,0,0.1)] sm:shadow-none z-30">
      ...
      <div className="bg-white dark:bg-gray-900 p-2 flex justify-between items-center border-b border-gray-300 dark:border-gray-800 shadow-sm z-10 gap-2">
        ...
        <div className="flex bg-gray-200 p-0.5 rounded-lg shadow-inner relative w-[72px] tutorial-toggle">
          ...
          <button onClick={() => setIsAddMode(true)} className={`flex-1 py-1.5 flex items-center justify-center font-black z-10 transition-colors ${isAddMode ? 'text-white' : 'text-gray-500 hover:text-gray-700'}`}>
            <Plus size={16} />
          </button>
          <button onClick={() => setIsAddMode(false)} className={`flex-1 py-1.5 flex items-center justify-center font-black z-10 transition-colors ${!isAddMode ? 'text-white' : 'text-gray-500 hover:text-gray-700'}`}>
            <Minus size={16} />
          </button>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 bg-gray-900 text-white rounded-lg font-bold text-[11px] tutorial-report-btn hover:bg-gray-800 transition-all shadow-sm active:scale-95"
        >
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-5xl flex flex-col max-h-[95vh] overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-200 dark:border-gray-800">
      ...
      <div className="bg-gray-900 text-white p-3 sm:p-4 flex justify-between items-center shadow-md z-10">
        ...
        <div className="flex gap-2">
          <button onClick={handleManualPaste} className="p-2 hover:bg-gray-800 rounded-lg transition-all flex items-center gap-1.5 text-xs text-blue-400 border border-blue-400/30">
            <ClipboardPaste size={18} /> <span className="hidden sm:inline">Paste</span>
          </button>
          <button onClick={() => setShowImportModal(true)} className="p-2 hover:bg-gray-800 rounded-lg transition-all flex items-center gap-1.5 text-xs text-green-400 border border-green-400/30">
            <Upload size={18} /> <span className="hidden sm:inline">Import</span>
          </button>
          <button onClick={() => setShowExportModal(true)} className="p-2 bg-yellow-400 text-yellow-900 rounded-lg transition-all flex items-center gap-1.5 text-xs font-black shadow-sm active:scale-95 border-b-2 border-yellow-600">
            ...
            {spreadsheetData.rows.map((row, idx) => (
              <tr key={idx} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="p-0 border-r border-gray-200 bg-blue-50/20 dark:bg-blue-50/5">
                  <input type="number" data-row={idx} data-field="starting"
                ...
                <td className="p-0 border-r border-gray-200 bg-blue-50/20 dark:bg-blue-50/5">
                  <input type="number" data-row={idx} data-field="deliver"
                ...
                <td className="p-0 border-r border-gray-200 bg-red-50/20 dark:bg-red-50/5">
                  <input type="number" data-row={idx} data-field="waste"
                ...
                <td className="p-0 border-r border-gray-200 bg-green-50/20 dark:bg-green-50/5">
                  <input type="number" data-row={idx} data-field="endingOverride"
                ...
                <td className={`p-2 border-r border-gray-200 text-center font-bold bg-gray-50 dark:bg-gray-800/40 transition-colors ${row.missingQty !== 0 ? 'text-purple-600 dark:text-purple-400' : 'text-blue-600 dark:text-blue-400'}`}>
                  {row.finalSold}
                </td>
      ...
      <div className="p-3 sm:p-4 bg-gray-100 dark:bg-gray-900 border-t dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-inner">
        ...
        <button onClick={handleClearDataClick} className="px-4 py-2 bg-red-600 text-white text-[10px] sm:text-xs font-bold rounded-lg hover:bg-red-700 transition-colors shadow-md">
...
export default function App() {
  ...
  const handleInventoryInput = (ingredient, field, value) => {
    ...
    setInventory(prev => ({
      ...prev,
      [ingredient]: { ...(prev[ingredient] || { starting: '', deliver: '', waste: '' }), [field]: numValue }
    }));
  };
  ...
  const handleManualPaste = async () => {
    ...
    } catch (err) { console.warn('Clipboard API blocked, falling back to manual entry', err); setPasteModal(true); }
  };
  ...
  return (
    <div className="flex flex-col sm:flex-row h-[100dvh] bg-gray-100 dark:bg-gray-900 font-sans overflow-hidden transition-colors" style={{ zoom: config.scale || 1 }}>
      ...
      <OrderFeed
        ...
        setDevContactModal={setDevContactModal}
        ordersContainerRef={ordersContainerRef}
        ordersEndRef={ordersEndRef}
        showScrollDown={showScrollDown}
        handleOrdersScroll={handleOrdersScroll}
      ...
      {showReport && (
        <ReportModal
          ...
          spreadsheetData={spreadsheetData}
          setShowReport={setShowReport}
          handleManualPaste={handleManualPaste}
          setShowImportModal={setShowImportModal}
          setShowExportModal={setShowExportModal}
          setLastActiveCell={setLastActiveCell}
          handleTableKeyDown={handleTableKeyDown}
          handleTablePaste={handleTablePaste}
          handleClearDataClick={handleClearDataClick}
        />
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col overflow-hidden animate-in zoom-in duration-200 border border-gray-200 dark:border-gray-800">
            ...
            <div className="p-6 dark:bg-gray-900">
              <input type="number" inputMode="numeric" autoFocus value={customValue}
                onChange={(e) => setCustomValue(e.target.value)} placeholder="Enter amount (₱)"
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-700 flex gap-3">
              <button onClick={() => { setCustomModal({ isOpen: false, isRemoving: false }); setCustomValue(''); }}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200 border border-gray-200 dark:border-gray-800 max-h-[90vh]">
            ...
            <div className="flex border-b dark:border-gray-800">
              {['general', 'prices', 'recipes'].map(tab => (
                <button key={tab} onClick={() => setSettingsTab(tab)}
                  className={`flex-1 py-3 text-sm font-bold transition-colors capitalize ${settingsTab === tab ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50 dark:bg-blue-900/20' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800'}`}>
                  {tab === 'general' ? 'General Config' : tab === 'prices' ? 'Item Prices (Overrides)' : 'Recipe Ingredients'}
                </button>
              ))}
            </div>
                    ...
                    <div className="flex gap-2">
                      {['12h', '24h'].map(fmt => (
                        <button key={fmt} onClick={() => setConfig(p => ({ ...p, timeFormat: fmt }))}
                          className={`flex-1 py-2 rounded-lg font-bold border-2 transition-all flex items-center justify-center gap-2 ${config.timeFormat === fmt ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400' : 'border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800'}`}>
                    ...
                    <div>
                      ...
                      {storeConfig.menuItems.filter(i => i.id !== 'custom_amount').map(item => (
                        <div key={item.id} className="flex justify-between items-center mb-1 bg-gray-50 dark:bg-gray-800 p-1 rounded">
                          ...
                          <input type="number" className="w-16 bg-white dark:bg-gray-900 border dark:border-gray-700 rounded p-1 text-xs text-right focus:outline-none focus:ring-1 focus:ring-blue-500 dark:text-white"
                            placeholder={item.price} value={config.customPrices.menu[item.id] ?? ''}
                    ...
                    <div>
                      ...
                      {Object.entries(storeConfig.inventoryDb).map(([name, defaultP]) => (
                        <div key={name} className="flex justify-between items-center mb-1 bg-gray-50 dark:bg-gray-800 p-1 rounded">
                          ...
                          <input type="number" className="w-16 bg-white dark:bg-gray-900 border dark:border-gray-700 rounded p-1 text-xs text-right focus:outline-none focus:ring-1 focus:ring-blue-500 dark:text-white"
                            placeholder={defaultP} value={config.customPrices.ingredients[name] ?? ''}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col overflow-hidden animate-in zoom-in duration-200 border border-gray-200 dark:border-gray-800">
            ...
            <div className="p-4 flex flex-col gap-3">
              ...
              <textarea autoFocus value={manualPasteText} onChange={(e) => setManualPasteText(e.target.value)} placeholder="Paste items here..."
            ...
            <div className="p-3 bg-gray-50 dark:bg-gray-800 border-t dark:border-gray-700 flex justify-end">
              <button onClick={() => { processPastedText(manualPasteText); setPasteModal(false); setManualPasteText(''); }}
            ...
            <div className="p-8 flex flex-col items-center justify-center text-center">
              ...
              <div className="mt-6 text-left w-full space-y-2">
                ...
                {['Active Orders & History', 'Inventory & Manual Counts', 'Shift Revenue Statistics'].map(item => (
                  <div key={item} className="flex items-center gap-2 text-[11px] text-gray-600 dark:text-gray-400 font-bold">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in duration-200 border border-gray-200 dark:border-gray-800">
            ...
            <div className="p-4 grid grid-cols-1 gap-2">
              {[
                ...
                <button key={opt.id} onClick={() => { exportData(opt.id); setShowExportModal(false); }}
